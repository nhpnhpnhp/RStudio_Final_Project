...
# Beta theo thời gian 
ggplot(subset_data %>% filter(!is.na(beta_est)), aes(x = Date)) +
...
ggplot(subset_data %>% filter(!is.na(gamma_est)), aes(x = Date)) +
...
# So sánh số ca nhiễm (filter out NA values)
ggplot() +
  geom_line(data = out, aes(x = time, y = I, color = "Mô hình")) +
  geom_line(data = subset_data %>% filter(!is.na(I)), 
            aes(x = 1:nrow(subset_data %>% filter(!is.na(I))), 
            y = I, color = "Thực tế")) +
  labs(title = "So sánh số ca nhiễm (I): mô hình vs thực tế", 
       x = "Ngày", y = "Số ca") +
  scale_color_manual(values = c("Mô hình" = "red", "Thực tế" = "blue")) +
  theme_minimal()
```
3.2 Tính toán các chỉ số theo yêu cầu
...
3.3 Tính tốc độ thay đổi số ca nhiễm trong khoảng thời gian
