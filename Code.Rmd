...
# Phân tích chuỗi thời gian và dự đoán ARIMA
print("Running chunk time_series_analysis")
# Kiểm tra gói forecast và ggplot2
if (!require(forecast)) {
  install.packages("forecast")
  library(forecast)
}
...
# Kiểm tra dữ liệu global_trend
if (!exists("global_trend")) {
  stop("global_trend không tồn tại. Chạy chunk eda_time_series trước.")
}
if (any(is.na(global_trend$TotalConfirmed))) {
  stop("Dữ liệu TotalConfirmed chứa NA.")
}
...
p1 <- ggplot(forecast_df, aes(x = Time, y = Cases, color = Type)) +
  ...
  scale_color_manual(values = c("Thực tế" = "#333333", "Dự đoán" = "#1E90FF")) +
